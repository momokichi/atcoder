...
int main() {
  int n;
  cin >> n;
  vector<pair<pair<ll, ll>, ll>> ad(n);
  rep(i, n) cin >> ad[i].first.first >> ad[i].first.second >> ad[i].second;
  rep(i, n) {
    ll a = ad[i].first.first;
    ll b = ad[i].first.second;
    ll c = ad[i].first.first + 1;
    ll d = ad[i].first.second + 1;
