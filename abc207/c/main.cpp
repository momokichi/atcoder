...
int main() {
  int n;
  cin >> n;
  vector<pair<int, pair<ll, ll>>> c(n);
  rep(i, n) cin >> c[i].first >> c[i].second.first >> c[i].second.second;
  ll ans = 0;
  sort(all(c), comp);
  for (int i = 0; i < n; ++i) {
    for (int j = i + 1; j < n; ++j) {
      if (c[i].first == 1 || c[i].first == 3) {
        if (c[j].first == 1 || c[j].first == 2) {
          if (c[i].second.second >= c[j].second.first) ans++;
        } else {
          if (c[i].second.second > c[j].second.first) ans++;
        }
      } else {
