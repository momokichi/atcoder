...
int main() {
  ...
  for (int k = 0; k < 4; k++) {
    bool flag = true;
    sort(all(s), comp);
    int diffx = s[0].x - t[0].x, diffy = s[0].y - t[0].y;
    for (int i = 0; i < n; ++i) {
      if (diffx != s[i].x - t[i].x || diffy != s[i].y - t[i].y) {
        flag = false;
        break;
      }
    }
    ...
    for (int i = 0; i < n; ++i) {
      int tmpx = s[i].x, tmpy = s[i].y;
      s[i].x = tmpx * cos(PI / 2) - tmpy * sin(PI / 2);
      s[i].y = tmpx * sin(PI / 2) + tmpy * cos(PI / 2);
    }
