...
int main() {
  ...
  rep(i, n) { cin >> houses[i].first >> houses[i].second; }

  sort(all(houses));

  ll a = abs(houses[0].first - houses[n - 1].first);

  sort(all(houses), comp);

  ll b = abs(houses[0].second - houses[n - 1].second);
  ...
  // cout << fifirsted << setprecision(9) << ans << endl;
}
